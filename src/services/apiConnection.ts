...
export class APIConnectionService {
  ...
  private constructor() {
    ...
    if (this.isProduction) {
      this.baseUrl =
        import.meta.env.VITE_API_URL_PRODUCTION || "https://api.omnidim.io";
    ...
    console.log(
      `🌐 API Connection Service initialized for ${this.status.environment} environment`
    );
  ...
  private async checkConnection(): Promise<void> {
    ...
    for (const url of urls) {
      try {
        ...
        const healthEndpoint = url.includes("omnidim.io")
          ? url
          : `${url}/api/health`;
        ...
        if (
          response.ok ||
          (url.includes("omnidim.io") && response.status < 500)
        ) {
        ...
        if (
          ...
          if (!this.isProduction) {
            console.log(`🔄 Development mode: Using fallback for ${url}`);
          } else {
            console.log(
              `🔄 CORS/Network issue detected, enabling fallback mode for ${url}`
            );
          }
    ...
    if (this.isProduction) {
      console.warn(
        `❌ All API Health Checks Failed (${this.status.retryCount + 1}/${
          this.maxRetries
        }):`,
        lastError?.message
      );
    ...
    if (
      ...
      console.log(
        `🔄 Enabling intelligent fallback mode due to network/CORS issues`
      );
  ...
  private showCriticalError(): void {
    console.error(
      "🚨 CRITICAL: API server is not responding after multiple attempts"
    );
    ...
    if (this.isProduction) {
      ...
      console.error(
        "   💡 Note: Using real omniverse.AI API endpoints (no local server needed)"
      );
    }
  ...
  public async makeRequest(
    ...
    if (!this.status.isConnected) {
      ...
      if (!connected) {
        console.log(
          `🔄 API not available, enabling fallback mode for ${endpoint}`
        );
    ...
    for (const url of urls) {
      try {
        ...
        lastError = error as Error;
        console.warn(
          `⚠️ API request failed for ${url}${endpoint}:`,
          error.message
        );
    ...
    this.enableFallbackMode(
      this.baseUrl,
      lastError?.message || "All endpoints failed"
    );
    return this.createFallbackResponse(endpoint, options);
  }
  ...
  private createFallbackResponse(
    ...
    if (
      ...
      throw new Error(
        "OmniDimension API is currently unavailable. Please try again later."
      );
