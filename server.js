...
// Middleware
app.use(cors({
  origin: ['http://localhost:5174', 'http://localhost:4173', 'https://omniverseai.netlify.app'],
  credentials: true
}));
...
function extractSearchTerms(query) {
  ...
  const categories = ['electronics', 'fashion', 'beauty', 'smartphones', 'laptops', 'shoes', 'clothing'];
...
function extractPriceRange(query) {
  const priceRegex = /(?:under|below|less than|up to)\s*(?:rs\.?|rupees?|₹)?\s*(\d+(?:,\d+)*)/i;
...
function calculateRelevanceScore(product, terms) {
  ...
  if (terms.brands.some(brand => product.brand.toLowerCase().includes(brand))) {
    score += 50;
  }
  
  // Category match
  if (terms.categories.some(cat => 
  ...
  // Price range match
  if (terms.priceRange && terms.priceRange.max && product.price <= terms.priceRange.max) {
    score += 20;
  }
...
app.get('/api/categories', (req, res) => {
  try {
    ...
    const subcategories = [...new Set(INDIAN_PRODUCTS.map(p => p.subcategory))];
