  ...
  try {
    ...
    for (const selector of selectors) {
      ...
      if (element && typeof element.click === 'function') {
        console.log(`🤖 Opening OmniDimension widget via selector: ${selector}`);
        element.click();
        return true;
      }
    }
    ...
    const iframe = document.querySelector('iframe[src*="omnidim"]') as HTMLIFrameElement;
    if (iframe) {
...
/**
 * Waits for the OmniDimension widget to load and then attempts to open it
 */
export const waitForOmniDimensionWidget = (maxWaitTime = 10000): Promise<boolean> => {
...
const setupWidgetDetection = () => {
  ...
  const widgetScript = document.querySelector('#omniverse-web-widget, #omnidimension-web-widget');
  if (widgetScript) {
    ...
    mutations.forEach((mutation) => {
      mutation.addedNodes.forEach((node) => {
        if (node.nodeType === Node.ELEMENT_NODE) {
          ...
          if (element.id?.includes('omnidim') ||
              element.className?.includes('omnidim') ||
              element.tagName === 'IFRAME' && element.getAttribute('src')?.includes('omnidim')) {
